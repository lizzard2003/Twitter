...
class TweetCellTableViewCell: UITableViewCell {
    ...
    @IBAction func favoriteTweet(_ sender: Any) {
        ...
        if (toBeFavorited){
            ...
            }, failure:{ (error) in  print("Favorite did not succeed")
        })
        } else {
    ...
    @IBAction func retweet(_ sender: Any) {
        TwitterAPICaller.client?.retweet(tweetId: tweetId, success:{ self.setRetweeted(true)}, failure: {(error) in print("Error n retweeting: \(error)")
        })
